...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # ── Visual: Data Flow ─────────────────────────────────────────────────────
    section("Data Flow",
            "How a raw CSV becomes a deployed prediction — visualized end to end")
    st.markdown(f"""
    <div style="background:{CARD};border:1px solid {BORDER};border-radius:10px;
                padding:22px 18px;margin-bottom:22px;overflow-x:auto">
      <svg viewBox="0 0 900 220" width="100%" style="display:block;min-width:780px;
        ...
        <g transform="translate(20,40)">
          <rect width="130" height="60" rx="6" fill="#1a1a1a" stroke="#333"/>
          <text x="65" y="22" fill="#888" text-anchor="middle" font-size="9" letter-spacing="2">SOURCE</text>
          <text x="65" y="40" fill="#fff" text-anchor="middle" font-size="12" font-weight="600">Billboard</text>
          <text x="65" y="54" fill="#666" text-anchor="middle" font-size="10">603 songs</text>
        </g>

        <!-- Source 2: TidyTuesday -->
        <g transform="translate(20,120)">
          <rect width="130" height="60" rx="6" fill="#1a1a1a" stroke="#333"/>
          <text x="65" y="22" fill="#888" text-anchor="middle" font-size="9" letter-spacing="2">SOURCE</text>
          <text x="65" y="40" fill="#fff" text-anchor="middle" font-size="12" font-weight="600">TidyTuesday</text>
          <text x="65" y="54" fill="#666" text-anchor="middle" font-size="10">32,833 songs</text>
        </g>

        <!-- Arrows to merge -->
        <line x1="150" y1="70" x2="200" y2="100" stroke="{GREEN}" stroke-width="1.5" marker-end="url(#arr)"/>
        <line x1="150" y1="150" x2="200" y2="120" stroke="{GREEN}" stroke-width="1.5" marker-end="url(#arr)"/>

        <!-- Merge -->
        <g transform="translate(205,85)">
          <rect width="120" height="50" rx="6" fill="rgba(29,185,84,0.08)" stroke="{GREEN}"/>
          <text x="60" y="22" fill="#fff" text-anchor="middle" font-size="11" font-weight="600">Merge + Clean</text>
          <text x="60" y="38" fill="{GREEN}" text-anchor="middle" font-size="10">24,993 songs</text>
        </g>

        <!-- Arrow to preprocess -->
        <line x1="325" y1="110" x2="365" y2="110" stroke="{GREEN}" stroke-width="1.5" marker-end="url(#arr)"/>

        <!-- Preprocess -->
        <g transform="translate(370,85)">
          <rect width="125" height="50" rx="6" fill="#1a1a1a" stroke="#333"/>
          <text x="62.5" y="22" fill="#fff" text-anchor="middle" font-size="11" font-weight="600">Preprocess</text>
          <text x="62.5" y="38" fill="#888" text-anchor="middle" font-size="9">Impute → Scale</text>
        </g>

        <!-- Arrow to split -->
        <line x1="495" y1="110" x2="535" y2="110" stroke="{GREEN}" stroke-width="1.5" marker-end="url(#arr)"/>

        <!-- Split -->
        <g transform="translate(540,75)">
          <rect width="100" height="35" rx="4" fill="rgba(29,185,84,0.1)" stroke="{GREEN}" stroke-width="1"/>
          <text x="50" y="22" fill="#fff" text-anchor="middle" font-size="10" font-weight="600">Train 70%</text>
          <rect y="42" width="100" height="25" rx="4" fill="#1a1a1a" stroke="#444"/>
          <text x="50" y="58" fill="#bbb" text-anchor="middle" font-size="10" font-weight="600">Test 30%</text>
        </g>

        <!-- Arrow to CV -->
        <line x1="640" y1="92" x2="685" y2="92" stroke="{GREEN}" stroke-width="1.5" marker-end="url(#arr)"/>

        <!-- CV -->
        <g transform="translate(690,75)">
          <rect width="90" height="35" rx="4" fill="#1a1a1a" stroke="#333"/>
          <text x="45" y="22" fill="#fff" text-anchor="middle" font-size="10" font-weight="600">4-Fold CV</text>
        </g>

        <!-- Arrow to model -->
        <line x1="735" y1="110" x2="735" y2="135" stroke="{GREEN}" stroke-width="1.5" marker-end="url(#arr)"/>

        <!-- Deployed Model -->
        <g transform="translate(660,140)">
          <rect width="220" height="50" rx="6" fill="rgba(29,185,84,0.15)" stroke="{GREEN}" stroke-width="1.5"/>
          <text x="110" y="22" fill="#fff" text-anchor="middle" font-size="11" font-weight="700">Random Forest (deployed)</text>
          <text x="110" y="38" fill="{GREEN}" text-anchor="middle" font-size="10">35.2% test acc · 12× chance</text>
        </g>

        <!-- Arrow from test -->
        <line x1="595" y1="140" x2="660" y2="160" stroke="#666" stroke-width="1" stroke-dasharray="3,3" marker-end="url(#arr)"/>
        <text x="610" y="158" fill="#666" font-size="9" font-style="italic">evaluate</text>
    ...
    section("4-Fold Cross-Validation",
            "Each fold trains on 75% and validates on the remaining 25% — repeated 4× for a robust estimate")
    st.markdown(f"""
    <div style="background:{CARD};border:1px solid {BORDER};border-radius:10px;
                padding:22px 24px;margin-bottom:22px">
      <svg viewBox="0 0 900 180" width="100%" style="display:block;font-family:system-ui">
        <!-- Labels -->
        <text x="60" y="20" fill="#888" font-size="10" letter-spacing="1.5">FOLD</text>
        <text x="430" y="20" fill="#888" font-size="10" letter-spacing="1.5">TRAINING SUBSET (75%)</text>
        <text x="770" y="20" fill="{GREEN}" font-size="10" letter-spacing="1.5">VALIDATION (25%)</text>

        <!-- Fold rows -->""" + "".join([
        f"""
        <text x="50" y="{45 + i*32}" fill="#fff" font-size="11" font-weight="600">{i+1}</text>
        <g transform="translate(80,{30 + i*32})">
          {"".join([
            f'<rect x="{j*180}" y="0" width="178" height="22" rx="3" '
            f'fill="{GREEN if j == i else "#2a2a2a"}" '
            f'opacity="{0.85 if j == i else 1}"/>'
            f'<text x="{j*180 + 89}" y="15" fill="{"#000" if j == i else "#888"}" '
            f'text-anchor="middle" font-size="9" font-weight="{"700" if j == i else "500"}">'
            f'{"VALIDATE" if j == i else "TRAIN"}</text>'
            for j in range(4)
          ])}
        </g>"""
        for i in range(4)
        ...
        <!-- Result summary -->
        <line x1="80" y1="160" x2="800" y2="160" stroke="#333" stroke-width="1"/>
        <text x="80" y="178" fill="#666" font-size="11" font-style="italic">
          Average across 4 folds = robust CV accuracy (e.g. RF: 40.8%)
        </text>
      </svg>
      <div style="font-size:0.78rem;color:#888;margin-top:6px;line-height:1.5">
        Stratified by genre so every fold contains every class proportionally.
        Without this, rare genres would sometimes be entirely absent from training.
      </div>
    </div>
    """, unsafe_allow_html=True)

    # ── Visual: Why audio-only (the OHE explosion) ────────────────────────────
    section("Why Audio-Only, Not Artist",
            "The cost of including artist identity as a feature")
    a1, a2 = st.columns(2)
    a1.markdown(f"""
    <div style="background:{CARD};border:1px solid {BORDER};border-radius:8px;padding:18px 20px;
                height:100%">
      <div style="font-size:0.62rem;color:#888;letter-spacing:0.14em;font-weight:700;
                  margin-bottom:8px">WITH ARTIST OHE</div>
      <div style="font-size:0.85rem;color:#fff;font-weight:600;margin-bottom:10px">
        ~4,400 columns
      </div>
      <svg viewBox="0 0 100 30" width="100%" height="30" preserveAspectRatio="none">
        <rect x="0" y="10" width="100" height="10" fill="#444"/>
        <rect x="0" y="10" width="0.25" height="10" fill="{GREEN}"/>
      </svg>
      <div style="font-size:0.7rem;color:#888;margin-top:8px;line-height:1.5">
        11 audio features + one-hot encoding of every unique artist (~4,400).
        High CV accuracy (43.7%) but the model is mostly learning
        <b style="color:#fff">artist → genre</b> shortcuts, not audio patterns.
      </div>
      <div style="margin-top:12px;padding-top:10px;border-top:1px solid #333">
        <span style="color:#ff7b7b;font-size:0.72rem;font-weight:600">
          Fails on any new/unseen artist
        </span>
      </div>
    </div>""", unsafe_allow_html=True)

    a2.markdown(f"""
    <div style="background:{CARD};border:1px solid {GREEN};border-radius:8px;padding:18px 20px;
                height:100%">
      <div style="font-size:0.62rem;color:{GREEN};letter-spacing:0.14em;font-weight:700;
                  margin-bottom:8px">AUDIO ONLY (DEPLOYED)</div>
      <div style="font-size:0.85rem;color:#fff;font-weight:600;margin-bottom:10px">
        11 columns
      </div>
      <svg viewBox="0 0 100 30" width="100%" height="30" preserveAspectRatio="none">
        <rect x="0" y="10" width="100" height="10" fill="#444"/>
        <rect x="0" y="10" width="100" height="10" fill="{GREEN}"/>
      </svg>
      <div style="font-size:0.7rem;color:#888;margin-top:8px;line-height:1.5">
        Just the 11 audio features. Lower CV (40.8%) and test (35.2%) accuracy,
        but every prediction is grounded in <b style="color:#fff">acoustic signal</b>,
        not memorized name lookups.
      </div>
      <div style="margin-top:12px;padding-top:10px;border-top:1px solid #333">
        <span style="color:{GREEN};font-size:0.72rem;font-weight:600">
          Generalises to any song the API can describe
        </span>
      </div>
    </div>""", unsafe_allow_html=True)
    ...
    section("The Class Imbalance Problem",
            "Why class_weight='balanced' is doing real work")
    counts = df_raw["top_genre"].value_counts()
    counts = counts[counts >= 5]
    max_c, min_c = int(counts.iloc[0]), int(counts.iloc[-1])
    bars_svg = "".join(
        f'<rect x="{i*22}" y="{100 - (c/max_c)*95}" width="18" '
        f'height="{(c/max_c)*95}" rx="2" '
        f'fill="{GREEN if c >= 500 else ("#666" if c >= 50 else "#3a3a3a")}" '
        f'opacity="{0.95 if c >= 500 else (0.7 if c >= 50 else 0.5)}"/>'
        for i, c in enumerate(counts.values)
    )
    st.markdown(f"""
    <div style="background:{CARD};border:1px solid {BORDER};border-radius:10px;
                padding:22px 24px;margin-bottom:22px">
      <svg viewBox="0 0 800 130" width="100%" style="display:block;font-family:system-ui">
        <g transform="translate(0,10)">{bars_svg}</g>
        <line x1="0" y1="110" x2="780" y2="110" stroke="#444" stroke-width="1"/>
        <text x="0" y="125" fill="#aaa" font-size="10" font-weight="600">
          dance pop ({max_c})
        </text>
        <text x="780" y="125" fill="#888" font-size="10" text-anchor="end">
          rare genres (5–10 songs each)
        </text>
      </svg>
      <div style="display:flex;gap:18px;margin-top:14px;font-size:0.74rem">
        <div style="display:flex;align-items:center;gap:6px;color:#888">
          <span style="width:10px;height:10px;background:{GREEN};border-radius:2px"></span>
          ≥ 500 songs · 8 classes
        </div>
        <div style="display:flex;align-items:center;gap:6px;color:#888">
          <span style="width:10px;height:10px;background:#666;border-radius:2px"></span>
          50–499 songs · ~15 classes
        </div>
        <div style="display:flex;align-items:center;gap:6px;color:#888">
          <span style="width:10px;height:10px;background:#3a3a3a;border-radius:2px"></span>
          5–49 songs · ~12 classes
        </div>
      </div>
      <div style="font-size:0.78rem;color:#888;margin-top:10px;line-height:1.5">
        Without balanced class weighting, the model would optimise for predicting
        dance pop on every input — it accounts for ~6% of all training data and is
        296× more common than the rarest classes.
      </div>
    </div>
    """, unsafe_allow_html=True)
